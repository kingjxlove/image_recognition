...
class LPR():
    ...
    def fine_mapping(self, image_rgb):
        ...
        for k in np.linspace(-50, 0, 16):
            ...
            if cv2.__version__[0] == "4":
                contours, hierarchy = cv2.findContours(binary_niblack.copy(), cv2.RETR_EXTERNAL,
    ...
    def affine_crop(self, image, pts):
        ...
        i_shape = image.shape
        x1, y1, x2, y2, x3, y3, x4, y4 = pts.ravel()
        min_x, max_x = min(x1, x2, x3, x4), (i_shape[1] - max(x1, x2, x3, x4))
        min_y, max_y = min(y1, y2, y3, y4), (i_shape[0] - max(y1, y2, y3, y4))
    ...
    def finetune(self, image_, stage=2):
        ...
        blob = cv2.dnn.blobFromImage(image, size=(120, 48), swapRB=False, mean=(127.5, 127.5, 127.5),
                                     scalefactor=0.0078125, crop=False)
        ...
        blob = cv2.dnn.blobFromImage(g, size=(120, 48), swapRB=False, mean=(127.5, 127.5, 127.5), scalefactor=0.0078125,
                                     crop=False)

        self.refine_net.setInput(blob)
        h, w, c = g.shape
        pts = (self.refine_net.forward("conv6-3").reshape(4, 2) * np.array([w, h])).astype(np.int)
        cropped, min_x, min_y, max_x, max_y = self.affine_crop(g, pts)
    ...
    def plate_recognition(self, image, minSize=30, charSelectionDeskew=True, mode='ssd'):
        ...
        for j, plate in enumerate(images):
            ...
            cropped, x1, y1, x2, y2 = self.loose_crop(image, [left, top, right, bottom], 120 / 48)
            cropped_finetuned, min_x, min_y, max_x, max_y = self.finetune(cropped)
            retval, buffer = cv2.imencode('.jpg', cropped_finetuned)
            pic_str = base64.b64encode(buffer)
            pic_str = pic_str.decode()
            res, confidence = self.segmentation_free_recognition(cropped_finetuned)
            # res_set.append([res, confidence, [x1 + min_x, y1 + min_y, x2 - max_x, y2 - max_y], image.shape, pic_str])
            #x1, y1, x2, y2 = int(x1 + min_x), int(y1 + min_y), int(x2 - max_x), int(y2 - max_y)
            res_set.append([res, confidence, [x1, y1, x2, y2], image.shape, pic_str])
        return res_set
